...
def subject(req):
    ...
    #print(resp.text)
    subject_list = resp.text.split("2018U")
    subject_list_serializer = []
    #print(subject_name_list)
    for i in range(0,len(subject_list)-1):
        subject_list_serializer.append({"sub_name" : subject_list[i+1].split("_ ")[1].split("(")[0],"sub_id":subject_list[i].split("Course&id=")[len(subject_list[i].split("Course&id="))-1].split("&")[0]})
